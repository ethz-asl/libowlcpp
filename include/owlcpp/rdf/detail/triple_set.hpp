...
public:
   ...
   friend bool operator<(Triple const& t1, Value_predicate const& vp) {
      ...
      if( at<Tag1>(t1)  < vp.q1_       ) return true;
      if( vp.q1_        < at<Tag1>(t1) ) return false;
      if( at<Tag2>(t1)  < vp.q2_       ) return true;
      if( vp.q2_        < at<Tag2>(t1) ) return false;
      return
               at<Tag3>(t1) < vp.q3_;
   }
...
> class Ts_query_dispatch {
   typedef Value_predicate<Q1,Q2,Q3> predicate;
