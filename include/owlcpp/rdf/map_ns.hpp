...
public:
   ...
   typedef map_ns::iterator iterator;
   typedef map_ns::const_iterator const_iterator;
   typedef map_ns::Err Err;

   template<class Nodes> explicit Map_ns(Nodes const& nodes)
   : smap_(Map_node_std::get(nodes)), map_(smap_.ns_id_next())
   {}

   explicit Map_ns(Map_node_std const& std_map)
