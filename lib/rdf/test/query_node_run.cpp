...
BOOST_AUTO_TEST_CASE( case01 ) {
   Triple_store ts;
   const Node_id nid1 = ts.insert_lit_node("1", to_string(T_xsd_boolean()));
   BOOST_CHECK( value<bool>(nid1, ts) );

   const Node_id nid2 = ts.insert_lit_node("true", to_string(T_xsd_boolean()));
   BOOST_CHECK( value<bool>(nid2, ts) );

   const Node_id nid3 = ts.insert_lit_node("TrUe", to_string(T_xsd_boolean()));
   BOOST_CHECK_THROW( value<bool>(nid3, ts), Rdf_err );

   const Node_id nid4 = ts.insert_lit_node("10e300", to_string(T_xsd_double()));
   ...
   const Node_id nid5 = ts.insert_lit_node("255", to_string(T_xsd_double()));
   ...
   const Node_id nid6 = ts.insert_lit_node("-10", to_string(T_xsd_double()));
